...
export default function Students() {
  ...
  useEffect(() => {
    const feat = async() => {
      setStudents(await Student.getAll())
    }
    feat()
  }, [])
  ...
  return (
    <>
      ...
      <form className="w-full flex justify-center gap-8 mb-8" onSubmit={(e) => e.preventDefault()}>
        ...
        <Button type="submit" className={`${search !== ""? "opacity-100 hover:bg-orange-100 hover:text-orange-500" : "duration-100 ease-out top[-2rem] opacity-40 hover:bg-orange-300 hover:text-orange-50"}  bg-orange-300`} onClick={() => setSearch("")}>Limpar Busca</Button>
      </form>
          ...
          filteredStudents?.map((student, key) => ( 
            <Card.root className="flex items-center justify-between w-full md:w-3/4 lg:w-[40rem] h-fit">
              <div className="flex items-center gap-5">
                <img src={student.avatar} className="w-12 h-14" alt={`picture of ${student.name || "[sem nome]"}`} />
                <h1>{student.name}</h1>
                <h1>{student.ra}</h1>
              </div>
